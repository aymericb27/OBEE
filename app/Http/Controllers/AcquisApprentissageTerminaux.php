...
class AcquisApprentissageTerminaux extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $autoGeneratedCode = empty($validated['code']);
        ...
        $aat = null;
        $attempts = $autoGeneratedCode ? 5 : 1;
        for ($i = 0; $i < $attempts; $i++) {
            if ($autoGeneratedCode && $i > 0) {
                $validated['code'] = $this->codeGen->nextAAT();
            }

            try {
                $aat = AAT::create($validated);
                break;
                ...
                if (!$autoGeneratedCode || !$isDuplicateCode) {
                    throw $exception;
                }
            }
        }
        ...
        if ($aat === null) {
            return response()->json([
                'success' => false,
                'message' => 'Impossible de generer un code AAT unique.',
            ], 422);
        }
